  ...
  main.innerHTML = `
    ...
    <div class="scroll">
      ...
      <div class="settings-card">
        ...
        <div class="hint">Optional. The review screen's "Next ST" button reserves the next Service Ticket number as a new row in the master tracking sheet, and each pushed order completes its row (Category, KAR Location, Status, Client Order #, Receiving Date, QB Invoice Number, Bill REGISTER). Needs: a native Google Sheet (an .xlsx in Drive must first be converted via File &gt; Save as Google Sheets — use the new sheet's URL), the Google Sheets API enabled in the same Google Cloud project as the Drive client, the sheet owned by (or shared with edit rights with) the connected Google account, and — for connections made before this feature — one reconnect below to grant the spreadsheet permission.</div>
